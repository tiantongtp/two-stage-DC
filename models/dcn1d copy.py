...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, dropout=0.1):
        ...
        self.conv1 = nn.Conv1d(in_planes, planes, kernel_size=1, bias=True)
        self.bn1 = nn.BatchNorm1d(planes) 
        self.conv2 = nn.Conv1d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=True)
        self.bn2 = nn.BatchNorm1d(planes) 
        self.conv3 = nn.Conv1d(planes, self.expansion*planes, kernel_size=1, bias=True)
...
class DCN1D(nn.Module):
    def __init__(self, depth, num_classes, dropout=0):
        super(DCN1D, self).__init__()
        self.in_planes = 64

        block, num_blocks = cfg(depth)
        self.conv1 = nn.Conv1d(1, self.in_planes, kernel_size=7, stride=2, padding=3,bias=False)##7*7 P3   5 2   3 1
        self.bn1 = nn.BatchNorm1d(self.in_planes)
        self.conv2 = nn.Conv1d(64, 64, kernel_size=3, stride=2, padding=1,bias=False)
